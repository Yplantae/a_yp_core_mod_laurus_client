...
class CategoryEditDialog extends StatefulWidget {
  final CategoryModel? category; // null이면 생성 모드
  final String? parentTitle;

  const CategoryEditDialog({Key? key, this.category, this.parentTitle}) : super(key: key);

  @override
  State<CategoryEditDialog> createState() => _CategoryEditDialogState();
}

class _CategoryEditDialogState extends State<CategoryEditDialog> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _selectedType = g?.type ?? CategoryType.general;
    _selectedStatus = g?.status ?? CategoryStatus.active;
  }
  ...
  Future<void> _onDelete() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text("Are you sure you want to delete '${widget.category?.title}'?\nThis action cannot be undone."),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    final title = isEditing ? 'Edit Category' : 'New Category';
        ...
        child: Column(
          children: [
            ...
            DropdownButtonFormField<CategoryStatus>(
              ...
              items: CategoryStatus.values.map((e) {
  ...
  Widget _buildMembersTab() {
    final members = widget.category?.memberIds ?? [];
    if (members.isEmpty) {
